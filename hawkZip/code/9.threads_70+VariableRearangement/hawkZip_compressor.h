...
#define NUM_THREADS 70   // Number of threads to use for parallel processing
...
*/
void hawkZip_compress_kernel(float* oriData, unsigned char* cmpData, int* absQuant, unsigned int* signFlag, int* fixedRate, unsigned int* threadOfs, const size_t nbEle, size_t* cmpSize, float errorBound)
{
   // Calculate chunk size for dividing work among threads
   const int chunk_size = (nbEle + NUM_THREADS - 1) / NUM_THREADS;
   const int block_num = (chunk_size+31)/32;          // Number of blocks per thread
   ...
   #pragma omp parallel
   {
       ...
       for(int i=0; i<block_num; i++)
       {
            ...
            for(int j=block_start; j<block_end; j++)
           {
                ...
                float data_scaled = oriData[j] * recip_precision;
                int s = data_scaled >= -0.5f ? 0 : 1;
                int curr_quant = (int)(data_scaled + 0.5f) - s;
               
                // Step 2: Store sign information in a bit flag
                int sign_ofs = j & 0x1F;
                int is_negative = curr_quant < 0;
                sign_flag |= (is_negative << (31 - sign_ofs));
               
                // Step 3: Store absolute value and track maximum
                int abs_quant = is_negative ? -curr_quant : curr_quant;
...
*/
void hawkZip_decompress_kernel(float* decData, unsigned char* cmpData, int* absQuant, int* fixedRate, unsigned int* threadOfs, const size_t nbEle, float errorBound)
{
   // Calculate chunk size for dividing work among threads
   const int chunk_size = (nbEle + NUM_THREADS - 1) / NUM_THREADS;
   const int block_num = (chunk_size+31)/32;          // Number of blocks per thread
